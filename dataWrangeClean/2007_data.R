...
age <- read.csv("2007_age.csv")
salary <- read.csv("hoops_hype_salary_2007.csv")
stat <- read.csv("2007_stats.csv")
...
#--------------------------#
miss_sal<-read.csv("missing_salary_2007.csv", header = F)
...
team_wins <- team_wins[,-c(2:13,15:21)]
...
a6<-grep("Elton Brand", test$player_name)
test[a6, 49] <- "YES"
a7<-grep("Tim Duncan", test$player_name)
test[a7, 49] <- "YES"
a8<-grep("Chauncey Billups", test$player_name)
test[a8, 49] <- "YES"
...
test[a13, 50] <- "YES"
a14<-grep("Allen Iverson", test$player_name)
test[a14, 50] <- "YES"
...
r1<-grep("Andrew Bogut", test$player_name)
test[r1, 51] <- "YES"
r2<-grep("Channing Frye", test$player_name)
test[r2, 51] <- "YES"
r3<-grep("Chris Paul", test$player_name)
test[r3, 51] <- "YES"
r4<-grep("Charlie Villanueva", test$player_name)
test[r4, 51] <- "YES"
r5<-grep("Deron Williams", test$player_name)
test[r5, 51] <- "YES"
...
r6<-grep("Deron Williams", test$player_name)
test[r6, 52] <- "YES"
r7<-grep("Ryan Gomes", test$player_name)
test[r7, 52] <- "YES"
r8<-grep("Danny Granger", test$player_name)
test[r8, 52] <- "YES"
r9<-grep("Luther Head", test$player_name)
test[r9, 52] <- "YES"
r10<-grep("Marvin Williams", test$player_name)
test[r10, 52] <- "YES"

#All defensive first team

d1<-grep("Metta World", test$player_name)
test[d1, 53] <- "YES"
...
test[d4, 53] <- "YES"
d5<-grep("Jason Kidd", test$player_name)
test[d5, 53] <- "YES"
...
test[d6, 54] <- "YES"
d7<-grep("Marcus Camby", test$player_name)
test[d7, 54] <- "YES"
d8<-grep("Kevin Garnett", test$player_name)
test[d8, 54] <- "YES"
d9<-grep("Tim Duncan", test$player_name)
test[d9, 54] <- "YES"
...
test[mvp, 43] <- "YES"
#MIP 44
mip<-grep("Boris Diaw", test$player_name)
test[mip, 44] <- "YES"
#6th man 45 
sixth<-grep("Mike Miller", test$player_name)
test[sixth, 45] <- "YES"
#DPOY 46
dpoy<-grep("Ben Wallace", test$player_name)
test[dpoy, 46] <- "YES"
#ROY 47
roy<-grep("Chris Paul", test$player_name)
test[roy, 47] <- "YES"
test<-format(test, digits = 11)



write.csv(test, file = "2007_data.csv")