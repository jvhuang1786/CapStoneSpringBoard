...
age <- read.csv("2008_age.csv")
salary <- read.csv("hoops_hype_salary_2008.csv")
stat <- read.csv("2008_stats.csv")
...
#--------------------------#
miss_sal<-read.csv("missing_salary_2008.csv", header = F)
...
team_wins <- team_wins[,-c(2:12,14:21)]
...
test[a1, 48] <- "YES"
a2<-grep("Dirk Nowitzki", test$player_name)
test[a2, 48] <- "YES" 
a3<-grep("Kobe Bryant", test$player_name)
test[a3, 48] <- "YES"
a4<-grep("Steve Nash", test$player_name)
test[a4,48] <- "YES"
a5<-grep("Amar'e Stoudemire", test$player_name)
test[a5, 48] <- "YES" 

#NBA second team 

a6<-grep("Chris Bosh", test$player_name)
test[a6, 49] <- "YES"
a7<-grep("LeBron James", test$player_name)
test[a7, 49] <- "YES"
a8<-grep("Gilbert Arenas", test$player_name)
test[a8, 49] <- "YES"
a9<-grep("Tracy McGrady", test$player_name)
test[a9, 49] <- "YES"
a10<-grep("Yao Ming", test$player_name)
test[a10, 49] <- "YES"
...
test[a11, 50] <- "YES"
a12 <-grep("Kevin Garnett", test$player_name)
test[a12, 50] <- "YES"
a13 <-grep("Chauncey Billups", test$player_name)
test[a13, 50] <- "YES"
a14<-grep("Dwyane Wade", test$player_name)
test[a14, 50] <- "YES"
...
r1<-grep("Andrea Bargnani", test$player_name)
test[r1, 51] <- "YES"
r2<-grep("Randy Foye", test$player_name)
test[r2, 51] <- "YES"
r3<-grep("Rudy Gay", test$player_name)
test[r3, 51] <- "YES"
r4<-grep("LaMarcus Aldridge", test$player_name)
test[r4, 51] <- "YES"
r5<-grep("Jorge Garbajosa", test$player_name)
test[r5, 51] <- "YES"
...
r6<-grep("Paul Millsap", test$player_name)
test[r6, 52] <- "YES"
r7<-grep("Adam Morrison", test$player_name)
test[r7, 52] <- "YES"
r8<-grep("Tyrus Thomas", test$player_name)
test[r8, 52] <- "YES"
r9<-grep("Walter Herrmann", test$player_name)
test[r9, 52] <- "YES"
r10<-grep("Rajon Rondo", test$player_name)
test[r10, 52] <- "YES"
r11<-grep("Marcus Williams", test$player_name)
test[r10, 52] <- "YES"
r13<-grep("Craig Smith", test$player_name)
test[r13, 52] <- "YES"
#All defensive first team

d1<-grep("Raja Bell", test$player_name)
test[d1, 53] <- "YES"
d2<-grep("Bruce Bowen", test$player_name)
test[d2, 53] <- "YES"
d3<-grep("Tim Duncan", test$player_name)
test[d3, 53] <- "YES"
...
d6<-grep("Kevin Garnett", test$player_name)
test[d6, 54] <- "YES"
d7<-grep("Kirk Hinrich", test$player_name)
test[d7, 54] <- "YES"
d8<-grep("Jason Kidd", test$player_name)
test[d8, 54] <- "YES"
d9<-grep("Tayshaun Prince", test$player_name)
test[d9, 54] <- "YES"
d10<-grep("Ben Wallace", test$player_name)
test[d10, 54] <- "YES"

#MVP 43
mvp<-grep("Dirk Nowitzki", test$player_name)
test[mvp, 43] <- "YES"
#MIP 44
mip<-grep("Monta Ellis", test$player_name)
test[mip, 44] <- "YES"
#6th man 45 
sixth<-grep("Leandro Barbosa", test$player_name)
test[sixth, 45] <- "YES"
#DPOY 46
dpoy<-grep("Marcus Camby", test$player_name)
test[dpoy, 46] <- "YES"
#ROY 47
roy<-grep("Brandon Roy", test$player_name)
test[roy, 47] <- "YES"
test<-format(test, digits = 11)



write.csv(test, file = "2008_data.csv")