...
age <- read.csv("2001_age.csv")
salary <- read.csv("hoops_hype_salary_2001.csv")
stat <- read.csv("2001_stats.csv")
...
#--------------------------#
miss_sal<-read.csv("missing_salary_2001.csv", header = F)
...
test[a1, 48] <- "YES"
a2<-grep("Gary Payton", test$player_name)
test[a2, 48] <- "YES" 
a3<-grep("Kevin Garnett", test$player_name)
test[a3, 48] <- "YES"
a4<-grep("Jason Kidd", test$player_name)
test[a4,48] <- "YES"
a5<-grep("Shaquille O'Neal", test$player_name)
test[a5, 48] <- "YES" 
...
test[a6, 49] <- "YES"
a7<-grep("Karl Malone", test$player_name)
test[a7, 49] <- "YES"
a8<-grep("Kobe Bryant", test$player_name)
test[a8, 49] <- "YES"
a9<-grep("Allen Iverson", test$player_name)
test[a9, 49] <- "YES"
a10<-grep("Alonzo Mourning", test$player_name)
test[a10, 49] <- "YES"

#NBA third team 

a11<-grep("Vince Carter", test$player_name)
test[a11, 50] <- "YES"
a12 <-grep("Chris Webber", test$player_name)
test[a12, 50] <- "YES"
a13 <-grep("Eddie Jones", test$player_name)
test[a13, 50] <- "YES"
a14<-grep("Stephon Marbury", test$player_name)
test[a14, 50] <- "YES"
a15<-grep("David Robinson", test$player_name)
test[a15, 50] <- "YES"

#All Rookie 1st team

r1<-grep("Elton Brand", test$player_name)
test[r1, 51] <- "YES"
r2<-grep("Steve Francis", test$player_name)
test[r2, 51] <- "YES"
r3<-grep("Andre Miller", test$player_name)
test[r3, 51] <- "YES"
r4<-grep("Lamar Odom", test$player_name)
test[r4, 51] <- "YES"
r5<-grep("Wally Szczerbiak", test$player_name)
test[r5, 51] <- "YES"
...
r6<-grep("Chucky Atkins", test$player_name)
test[r6, 52] <- "YES"
r7<-grep("Shawn Marion", test$player_name)
test[r7, 52] <- "YES"
r8<-grep("James Posey", test$player_name)
test[r8, 52] <- "YES"
r9<-grep("Jason Terry", test$player_name)
test[r9, 52] <- "YES"
r10<-grep("Metta World Peace", test$player_name)
test[r10, 52] <- "YES"
#r11<-grep("Vladimir Radmanovic", test$player_name)
#test[r11, 52] <- "YES"
#All defensive first team

d1<-grep("Kobe Bryant", test$player_name)
test[d1, 53] <- "YES"
d2<-grep("Kevin Garnett", test$player_name)
test[d2, 53] <- "YES"
d3<-grep("Alonzo Mourning", test$player_name)
test[d3, 53] <- "YES"
d4<-grep("Gary Payton", test$player_name)
test[d4, 53] <- "YES"
d5<-grep("Tim Duncan", test$player_name)
test[d5, 53] <- "YES"
#d11<-grep("Andrei Kirilenko", test$player_name)
#test[d11, 53] <- "YES"
#All defensive second team 

d6<-grep("Eddie Jones", test$player_name)
test[d6, 54] <- "YES"
d7<-grep("Jason Kidd", test$player_name)
test[d7, 54] <- "YES"
d8<-grep("Shaquille O'Neal", test$player_name)
test[d8, 54] <- "YES"
d9<-grep("Scottie Pippen", test$player_name)
test[d9, 54] <- "YES"
d10<-grep("Clifford Robinson", test$player_name)
test[d10, 54] <- "YES"
#d11<-grep("Dwyane Wade", test$player_name)
#test[d11, 54] <- "YES"
#MVP 43
mvp<-grep("Shaquille O'Neal ", test$player_name)
test[mvp, 43] <- "YES"
#MIP 44
mip<-grep("Jalen Rose", test$player_name)
test[mip, 44] <- "YES"
#6th man 45 
sixth<-grep("Rodney Rogers", test$player_name)
test[sixth, 45] <- "YES"
#DPOY 46
dpoy<-grep("Alonzo Mourning", test$player_name)
test[dpoy, 46] <- "YES"
#ROY 47
roy<-grep("Elton Brand", test$player_name)
test[roy, 47] <- "YES"
roy2<-grep("Steve Francis", test$player_name)
test[roy, 47] <- "YES"
test<-format(test, digits = 11)

write.csv(test, file = "2001_data.csv")