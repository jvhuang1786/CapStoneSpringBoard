...
age <- read.csv("2011_age.csv")
salary <- read.csv("hoops_hype_salary_2011.csv")
stat <- read.csv("2011_stats.csv")
...
#--------------------------#
miss_sal<-read.csv("missing_salary_2011.csv", header = F)
...
team_wins <- team_wins[,-c(2:9,11:21)]
...
test[a3, 48] <- "YES"
a4<-grep("Dwyane Wade", test$player_name)
test[a4,48] <- "YES"
...
a6<-grep("Steve Nash", test$player_name)
test[a6, 49] <- "YES"
a7<-grep("Dirk Nowitzki", test$player_name)
test[a7, 49] <- "YES"
a8<-grep("Carmelo Anthony", test$player_name)
test[a8, 49] <- "YES"
a9<-grep("Deron Williams", test$player_name)
test[a9, 49] <- "YES"
...
a11<-grep("Tim Duncan", test$player_name)
test[a11, 50] <- "YES"
a12 <-grep("Pau Gasol", test$player_name)
test[a12, 50] <- "YES"
a13 <-grep("Joe Johnson", test$player_name)
test[a13, 50] <- "YES"
a14<-grep("Brandon Roy", test$player_name)
test[a14, 50] <- "YES"
a15<-grep("Andrew Bogut", test$player_name)
test[a15, 50] <- "YES"

#All Rookie 1st team

r1<-grep("Darren Collison", test$player_name)
test[r1, 51] <- "YES"
r2<-grep("Stephen Curry", test$player_name)
test[r2, 51] <- "YES"
r3<-grep("Tyreke Evans", test$player_name)
test[r3, 51] <- "YES"
r4<-grep("Taj Gibson", test$player_name)
test[r4, 51] <- "YES"
r5<-grep("Brandon Jennings", test$player_name)
test[r5, 51] <- "YES"
...
r6<-grep("DeJuan Blair", test$player_name)
test[r6, 52] <- "YES"
r7<-grep("Jonny Flynn", test$player_name)
test[r7, 52] <- "YES"
r8<-grep("James Harden", test$player_name)
test[r8, 52] <- "YES"
r9<-grep("Jonas Jerebko", test$player_name)
test[r9, 52] <- "YES"
r10<-grep("Marcus Thornton", test$player_name)
test[r10, 52] <- "YES"
...
test[d1, 53] <- "YES"
d2<-grep("Gerald Wallace", test$player_name)
test[d2, 53] <- "YES"
...
test[d6, 54] <- "YES"
d7<-grep("Thabo Sefolosha", test$player_name)
test[d7, 54] <- "YES"
d8<-grep("Josh Smith", test$player_name)
test[d8, 54] <- "YES"
d9<-grep("Anderson Varejao", test$player_name)
test[d9, 54] <- "YES"
d10<-grep("Dwyane Wade", test$player_name)
test[d10, 54] <- "YES"
...
test[mvp, 43] <- "YES"
#MIP 44
mip<-grep("Aaron Brooks", test$player_name)
test[mip, 44] <- "YES"
#6th man 45 
...
test[dpoy, 46] <- "YES"
#ROY 47
roy<-grep("Tyreke Evans", test$player_name)
test[roy, 47] <- "YES"

test<-format(test, digits = 11)

write.csv(test, file = "2011_data.csv")