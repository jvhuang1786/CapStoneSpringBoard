...
age <- read.csv("2005_age.csv")
salary <- read.csv("hoops_hype_salary_2005.csv")
stat <- read.csv("2005_stats.csv")
...
#--------------------------#
miss_sal<-read.csv("missing_salary_2005.csv", header = F)
...
team_wins <- team_wins[,-c(2:15,17:21)]
...
a6<-grep("Jermaine O'Neal", test$player_name)
test[a6, 49] <- "YES"
a7<-grep("Peja Stojakovic", test$player_name)
test[a7, 49] <- "YES"
a8<-grep("Sam Cassell", test$player_name)
test[a8, 49] <- "YES"
a9<-grep("Tracy McGrady", test$player_name)
test[a9, 49] <- "YES"
...
a11<-grep("Dirk Nowitzki", test$player_name)
test[a11, 50] <- "YES"
a12 <-grep("Metta World Peace", test$player_name)
test[a12, 50] <- "YES"
a13 <-grep("Michael Redd", test$player_name)
test[a13, 50] <- "YES"
a14<-grep("Baron Davis", test$player_name)
test[a14, 50] <- "YES"
...
r1<-grep("Carmelo Anthony", test$player_name)
test[r1, 51] <- "YES"
r2<-grep("Chris Bosh", test$player_name)
test[r2, 51] <- "YES"
r3<-grep("Kirk Hinrich", test$player_name)
test[r3, 51] <- "YES"
r4<-grep("LeBron James", test$player_name)
test[r4, 51] <- "YES"
r5<-grep("Dwyane Wade", test$player_name)
test[r5, 51] <- "YES"
...
r6<-grep("Marquis Daniels", test$player_name)
test[r6, 52] <- "YES"
r7<-grep("T.J. Ford", test$player_name)
test[r7, 52] <- "YES"
r8<-grep("Udonis Haslem", test$player_name)
test[r8, 52] <- "YES"
r9<-grep("Jarvis Hayes", test$player_name)
test[r9, 52] <- "YES"
r10<-grep("Josh Howard", test$player_name)
test[r10, 52] <- "YES"

#All defensive first team

d1<-grep("Metta World Peace", test$player_name)
test[d1, 53] <- "YES"
...
test[d4, 53] <- "YES"
d5<-grep("Bruce Bowen", test$player_name)
test[d5, 53] <- "YES"
#d11<-grep("Andrei Kirilenko", test$player_name)
#test[d11, 53] <- "YES"
#All defensive second team 

d6<-grep("Doug Christie", test$player_name)
test[d6, 54] <- "YES"
d7<-grep("Tim Duncan", test$player_name)
test[d7, 54] <- "YES"
d8<-grep("Jason Kidd", test$player_name)
test[d8, 54] <- "YES"
d9<-grep("Theo Ratliff", test$player_name)
test[d9, 54] <- "YES"
d10<-grep("Andrei Kirilenko", test$player_name)
test[d10, 54] <- "YES"
#d11<-grep("Dwyane Wade", test$player_name)
#test[d11, 54] <- "YES"
#MVP 43
mvp<-grep("Kevin Garnett", test$player_name)
test[mvp, 43] <- "YES"
#MIP 44
mip<-grep("Zach Randolph", test$player_name)
test[mip, 44] <- "YES"
#6th man 45 
sixth<-grep("Antawn Jamison", test$player_name)
test[sixth, 45] <- "YES"
#DPOY 46
dpoy<-grep("Metta World Peace", test$player_name)
test[dpoy, 46] <- "YES"
#ROY 47
roy<-grep("LeBron James", test$player_name)
test[roy, 47] <- "YES"
test<-format(test, digits = 11)



write.csv(test, file = "2005_data.csv")