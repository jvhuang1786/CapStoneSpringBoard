...
age <- read.csv("2006_age.csv")
salary <- read.csv("hoops_hype_salary_2006.csv")
stat <- read.csv("2006_stats.csv")
...
#--------------------------#
miss_sal<-read.csv("missing_salary_2006.csv", header = F)
...
team_wins <- team_wins[,-c(2:14,16:21)]
...
a6<-grep("Kevin Garnett", test$player_name)
test[a6, 49] <- "YES"
a7<-grep("LeBron James", test$player_name)
test[a7, 49] <- "YES"
a8<-grep("Ray Allen", test$player_name)
test[a8, 49] <- "YES"
a9<-grep("Dwyane Wade", test$player_name)
test[a9, 49] <- "YES"
...
a11<-grep("Kobe Bryant", test$player_name)
test[a11, 50] <- "YES"
a12 <-grep("Shawn Marion", test$player_name)
test[a12, 50] <- "YES"
a13 <-grep("Gilbert Arenas", test$player_name)
test[a13, 50] <- "YES"
a14<-grep("Tracy McGrady", test$player_name)
test[a14, 50] <- "YES"
a15<-grep("Ben Wallace", test$player_name)
test[a15, 50] <- "YES"

#All Rookie 1st team

r1<-grep("Luol Deng", test$player_name)
test[r1, 51] <- "YES"
r2<-grep("Ben Gordon", test$player_name)
test[r2, 51] <- "YES"
r3<-grep("Dwight Howard", test$player_name)
test[r3, 51] <- "YES"
r4<-grep("Andre Iguodala", test$player_name)
test[r4, 51] <- "YES"
r5<-grep("Emeka Okafor", test$player_name)
test[r5, 51] <- "YES"
...
r6<-grep("Josh Childress", test$player_name)
test[r6, 52] <- "YES"
r7<-grep("Al Jefferson", test$player_name)
test[r7, 52] <- "YES"
r8<-grep("Nenad Krstic", test$player_name)
test[r8, 52] <- "YES"
r9<-grep("Jameer Nelson", test$player_name)
test[r9, 52] <- "YES"
r10<-grep("Josh Smith", test$player_name)
test[r10, 52] <- "YES"
...
test[d3, 53] <- "YES"
d4<-grep("Larry Hughes", test$player_name)
test[d4, 53] <- "YES"
...
d6<-grep("Chauncey Billups", test$player_name)
test[d6, 54] <- "YES"
d7<-grep("Andrei Kirilenko", test$player_name)
test[d7, 54] <- "YES"
d8<-grep("Kevin Garnett", test$player_name)
test[d8, 54] <- "YES"
d9<-grep("Tayshaun Prince", test$player_name)
test[d9, 54] <- "YES"
d10<-grep("Jason Kidd", test$player_name)
test[d10, 54] <- "YES"
d11<-grep("Dwyane Wade", test$player_name)
test[d11, 54] <- "YES"
#MVP 43
mvp<-grep("Steve Nash", test$player_name)
test[mvp, 43] <- "YES"
#MIP 44
mip<-grep("Bobby Simmons", test$player_name)
test[mip, 44] <- "YES"
#6th man 45 
sixth<-grep("Ben Gordon", test$player_name)
test[sixth, 45] <- "YES"
#DPOY 46
dpoy<-grep("Ben Wallace", test$player_name)
test[dpoy, 46] <- "YES"
#ROY 47
roy<-grep("Emeka Okafor", test$player_name)
test[roy, 47] <- "YES"
test<-format(test, digits = 11)



write.csv(test, file = "2006_data.csv")