...
age <- read.csv("2003_age.csv")
salary <- read.csv("hoops_hype_salary_2003.csv")
stat <- read.csv("2003_stats.csv")
...
#--------------------------#
miss_sal<-read.csv("missing_salary_2003.csv", header = F)
...
team_wins <- team_wins[,-c(2:17,19:21)]
...
test[a1, 48] <- "YES"
a2<-grep("Tracy McGrady", test$player_name)
test[a2, 48] <- "YES" 
...
test[a8, 49] <- "YES"
a9<-grep("Kevin Garnett", test$player_name)
test[a9, 49] <- "YES"
a10<-grep("Gary Payton", test$player_name)
test[a10, 49] <- "YES"

#NBA third team 

a11<-grep("Jermaine O'Neal", test$player_name)
test[a11, 50] <- "YES"
a12 <-grep("Paul Pierce", test$player_name)
test[a12, 50] <- "YES"
a13 <-grep("Ben Wallace", test$player_name)
test[a13, 50] <- "YES"
a14<-grep("Steve Nash", test$player_name)
test[a14, 50] <- "YES"
a15<-grep("Dikembe Mutombo", test$player_name)
test[a15, 50] <- "YES"

#All Rookie 1st team

r1<-grep("Shane Battier", test$player_name)
test[r1, 51] <- "YES"
r2<-grep("Pau Gasol", test$player_name)
test[r2, 51] <- "YES"
r3<-grep("Andrei Kirilenko", test$player_name)
test[r3, 51] <- "YES"
r4<-grep("Tony Parker", test$player_name)
test[r4, 51] <- "YES"
r5<-grep("Jason Richardson", test$player_name)
test[r5, 51] <- "YES"
...
r6<-grep("Eddie Griffi", test$player_name)
test[r6, 52] <- "YES"
r7<-grep("Richard Jefferson", test$player_name)
test[r7, 52] <- "YES"
r8<-grep("Zeljko Rebraca", test$player_name)
test[r8, 52] <- "YES"
r9<-grep("Jamaal Tinsley", test$player_name)
test[r9, 52] <- "YES"
r10<-grep("Joe Johnson", test$player_name)
test[r10, 52] <- "YES"
r11<-grep("Vladimir Radmanovic", test$player_name)
test[r11, 52] <- "YES"
#All defensive first team

d1<-grep("Gary Payton", test$player_name)
test[d1, 53] <- "YES"
d2<-grep("Kevin Garnett", test$player_name)
test[d2, 53] <- "YES"
d3<-grep("Ben Wallace", test$player_name)
test[d3, 53] <- "YES"
d4<-grep("Jason Kidd", test$player_name)
test[d4, 53] <- "YES"
...
test[d6, 54] <- "YES"
d7<-grep("Kobe Bryant", test$player_name)
test[d7, 54] <- "YES"
d8<-grep("Doug Christie", test$player_name)
test[d8, 54] <- "YES"
...
test[mvp, 43] <- "YES"
#MIP 44
mip<-grep("Jermaine O'Neal", test$player_name)
test[mip, 44] <- "YES"
#6th man 45 
sixth<-grep("Corliss Williamson", test$player_name)
test[sixth, 45] <- "YES"
#DPOY 46
dpoy<-grep("Ben Wallace", test$player_name)
test[dpoy, 46] <- "YES"
#ROY 47
roy<-grep("Pau Gasol", test$player_name)
test[roy, 47] <- "YES"
test<-format(test, digits = 11)

write.csv(test, file = "2003_data.csv")