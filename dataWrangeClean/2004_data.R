...
age <- read.csv("2004_age.csv")
salary <- read.csv("hoops_hype_salary_2004.csv")
stat <- read.csv("2004_stats.csv")
...
#--------------------------#
miss_sal<-read.csv("missing_salary_2004.csv", header = F)
...
team_wins <- team_wins[,-c(2:16,18:21)]
...
test[a1, 48] <- "YES"
a2<-grep("Kevin Garnett", test$player_name)
test[a2, 48] <- "YES" 
a3<-grep("Kobe Bryant", test$player_name)
test[a3, 48] <- "YES"
a4<-grep("Tracy McGrady", test$player_name)
test[a4,48] <- "YES"
...
a6<-grep("Dirk Nowitzki", test$player_name)
test[a6, 49] <- "YES"
a7<-grep("Chris Webber", test$player_name)
test[a7, 49] <- "YES"
a8<-grep("Allen Iverson", test$player_name)
test[a8, 49] <- "YES"
a9<-grep("Jason Kidd", test$player_name)
test[a9, 49] <- "YES"
a10<-grep("Ben Wallace", test$player_name)
test[a10, 49] <- "YES"

#NBA third team 

a11<-grep("Jamal Mashburn", test$player_name)
test[a11, 50] <- "YES"
a12 <-grep("Paul Pierce", test$player_name)
test[a12, 50] <- "YES"
a13 <-grep("Stephon Marbury", test$player_name)
test[a13, 50] <- "YES"
a14<-grep("Steve Nash", test$player_name)
test[a14, 50] <- "YES"
a15<-grep("Jermaine O'Neal", test$player_name)
test[a15, 50] <- "YES"

#All Rookie 1st team

r1<-grep("Caron Butler", test$player_name)
test[r1, 51] <- "YES"
r2<-grep("Drew Gooden", test$player_name)
test[r2, 51] <- "YES"
r3<-grep("Nene Hilario", test$player_name)
test[r3, 51] <- "YES"
r4<-grep("Yao Ming", test$player_name)
test[r4, 51] <- "YES"
r5<-grep("Amar'e Stoudemire", test$player_name)
test[r5, 51] <- "YES"
...
r6<-grep("Carlos Boozer", test$player_name)
test[r6, 52] <- "YES"
r7<-grep("JR Bremer", test$player_name)
test[r7, 52] <- "YES"
r8<-grep("Manu Ginobili", test$player_name)
test[r8, 52] <- "YES"
r9<-grep("Gordan Giricek", test$player_name)
test[r9, 52] <- "YES"
r10<-grep("Jay Williams", test$player_name)
test[r10, 52] <- "YES"
...
test[d2, 53] <- "YES"
d3<-grep("Ben Wallace", test$player_name)
test[d3, 53] <- "YES"
d4<-grep("Doug Christie", test$player_name)
test[d4, 53] <- "YES"
...
d6<-grep("Bruce Bowen", test$player_name)
test[d6, 54] <- "YES"
d7<-grep("Jason Kidd", test$player_name)
test[d7, 54] <- "YES"
d8<-grep("Shaquille O'Neal", test$player_name)
test[d8, 54] <- "YES"
d9<-grep("Eric Snow", test$player_name)
test[d9, 54] <- "YES"
d10<-grep("Metta World Peace", test$player_name)
test[d10, 54] <- "YES"
#d11<-grep("Dwyane Wade", test$player_name)
#test[d11, 54] <- "YES"
#MVP 43
mvp<-grep("Tim Duncan", test$player_name)
test[mvp, 43] <- "YES"
#MIP 44
mip<-grep("Gilbert Arenas", test$player_name)
test[mip, 44] <- "YES"
#6th man 45 
sixth<-grep("Bobby Jackson", test$player_name)
test[sixth, 45] <- "YES"
#DPOY 46
dpoy<-grep("Ben Wallace", test$player_name)
test[dpoy, 46] <- "YES"
#ROY 47
roy<-grep("Amar'e Stoudemire", test$player_name)
test[roy, 47] <- "YES"
test<-format(test, digits = 11)

write.csv(test, file = "2004_data.csv")