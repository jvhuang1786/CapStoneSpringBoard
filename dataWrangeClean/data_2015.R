...
age <- read.csv("2015_age.csv", header = F)
salary <- read.csv("hoops_hype_salary_2015.csv")
stat <- read.csv("2015_stats.csv")
...
age <- age[,-c(16:20)]
age <- age[,-c(11:14)]
colnames(age)[1:11]<-c("PLAYER",	"TEAM",	"AGE",	"HEIGHT",	"WEIGHT",	
             "COLLEGE",	"COUNTRY",	"DRAFT", 
             "ROUND",	 "NUMBER",
             "NETRTG")
...
#testing stat 
#Glen Rice Jr, Otto Porter Jr Tim Hardaway Jr 
...
#--------------------------#
miss_sal<-read.csv("miss_salary_2015.csv", header = F)
...
colnames(data)[8:10]<- c("DRAFT.YEAR","DRAFT.ROUND", "DRAFT.NUMBER")
data <- data[-c(514:536),]
...
team_wins <- team_wins[,-c(2:5,7:21)]
...
test[a2, 48] <- "YES" 
a3<-grep("Joakim Noah", test$player_name)
test[a3, 48] <- "YES"
a4<-grep("Kevin Durant", test$player_name)
test[a4,48] <- "YES"
a5<-grep("James Harden", test$player_name)
test[a5, 48] <- "YES" 

#NBA second team 
a6<-grep("Blake Griffin", test$player_name)
test[a6, 49] <- "YES"
a7<-grep("Kevin Love", test$player_name)
test[a7, 49] <- "YES"
a8<-grep("Dwight Howard", test$player_name)
test[a8, 49] <- "YES"
a9<-grep("Stephen Curry", test$player_name)
test[a9, 49] <- "YES"
a10<-grep("Tony Parker", test$player_name)
test[a10, 49] <- "YES"

#NBA third team 
a11<-grep("Paul George", test$player_name)
test[a11, 50] <- "YES"
a12 <-grep("LaMarcus Aldridge", test$player_name)
test[a12, 50] <- "YES"
a13 <-grep("Al Jefferson", test$player_name)
test[a13, 50] <- "YES"
a14<-grep("Goran Dragic", test$player_name)
test[a14, 50] <- "YES"
a15<-grep("Damian Lillard", test$player_name)
test[a15, 50] <- "YES"

#All Rookie 1st team
r1<-grep("Trey Burke", test$player_name)
test[r1, 51] <- "YES"
r2<-grep("Mason Plumlee", test$player_name)
test[r2, 51] <- "YES"
r3<-grep("Tim Hardaway", test$player_name)
test[r3, 51] <- "YES"
r4<-grep("Michael Carter-Williams", test$player_name)
test[r4, 51] <- "YES"
r5<-grep("Victor Oladipo", test$player_name)
test[r5, 51] <- "YES"


#All rookie Second Team
r6<-grep("Kelly Olynyk", test$player_name)
test[r6, 52] <- "YES"
r7<-grep("Giannis Antetokounmpo", test$player_name)
test[r7, 52] <- "YES"
r8<-grep("Gorgui Dieng", test$player_name)
test[r8, 52] <- "YES"
r9<-grep("Cody Zeller", test$player_name)
test[r9, 52] <- "YES"
r10<-grep("Steven Adams", test$player_name)
test[r10, 52] <- "YES"

#All defensive first team
d1<-grep("Joakim Noah", test$player_name)
test[d1, 53] <- "YES"
d2<-grep("Serge Ibaka", test$player_name)
test[d2, 53] <- "YES"
d3<-grep("Andre Iguodala", test$player_name)
test[d3, 53] <- "YES"
d4<-grep("Chris Paul", test$player_name)
test[d4, 53] <- "YES"
d5<-grep("Paul George", test$player_name)
test[d5, 53] <- "YES"
#All defensive second team 
d6<-grep("LeBron James", test$player_name)
test[d6, 54] <- "YES"
d7<-grep("Patrick Beverley", test$player_name)
test[d7, 54] <- "YES"
d8<-grep("Roy Hibbert", test$player_name)
test[d8, 54] <- "YES"
d9<-grep("Kawhi Leonard", test$player_name)
test[d9, 54] <- "YES"
d10<-grep("Jimmy Butler", test$player_name)
test[d10, 54] <- "YES"

#MVP 43
mvp<-grep("Kevin Durant", test$player_name)
test[mvp, 43] <- "YES"
#MIP 44
mip<-grep("Goran Dragic", test$player_name)
test[mip, 44] <- "YES"
#6th man 45 
sixth<-grep("Jamal Crawford", test$player_name)
test[sixth, 45] <- "YES"
#DPOY 46
dpoy<-grep("Joakim Noah", test$player_name)
test[dpoy, 46] <- "YES"
#ROY 47
roy<-grep("Michael Carter-Williams", test$player_name)
test[roy, 47] <- "YES"

test<-format(test, digits = 11)

write.csv(test, file = "2015_data.csv")