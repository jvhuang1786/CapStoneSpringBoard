...
age <- read.csv("2002_age.csv")
salary <- read.csv("hoops_hype_salary_2002.csv")
stat <- read.csv("2002_stats.csv")
...
#--------------------------#
miss_sal<-read.csv("missing_salary_2002.csv", header = F)
...
team_wins <- team_wins[,-c(2:18,20:21)]
...
test[a1, 48] <- "YES"
a2<-grep("Chris Webber", test$player_name)
test[a2, 48] <- "YES" 
...
a6<-grep("Vince Carter", test$player_name)
test[a6, 49] <- "YES"
a7<-grep("Kevin Garnett", test$player_name)
test[a7, 49] <- "YES"
a8<-grep("Kobe Bryant", test$player_name)
test[a8, 49] <- "YES"
a9<-grep("Tracy McGrady", test$player_name)
test[a9, 49] <- "YES"
a10<-grep("Dikembe Mutombo", test$player_name)
test[a10, 49] <- "YES"

#NBA third team 

a11<-grep("Karl Malone", test$player_name)
test[a11, 50] <- "YES"
a12 <-grep("Dirk Nowitzki", test$player_name)
test[a12, 50] <- "YES"
a13 <-grep("Ray Allen", test$player_name)
test[a13, 50] <- "YES"
a14<-grep("Gary Payton", test$player_name)
test[a14, 50] <- "YES"
...
r1<-grep("Marc Jackson", test$player_name)
test[r1, 51] <- "YES"
r2<-grep("Kenyon Martin", test$player_name)
test[r2, 51] <- "YES"
r3<-grep("Darius Miles", test$player_name)
test[r3, 51] <- "YES"
r4<-grep("Mike Miller", test$player_name)
test[r4, 51] <- "YES"
r5<-grep("Morris Peterson", test$player_name)
test[r5, 51] <- "YES"
...
r6<-grep("Courtney Alexander", test$player_name)
test[r6, 52] <- "YES"
r7<-grep("Marcus Fizer", test$player_name)
test[r7, 52] <- "YES"
r8<-grep("Desmond Mason", test$player_name)
test[r8, 52] <- "YES"
r9<-grep("Chris Mihm", test$player_name)
test[r9, 52] <- "YES"
r10<-grep("Hedo Turkoglu", test$player_name)
test[r10, 52] <- "YES"
...
test[d3, 53] <- "YES"
d4<-grep("Dikembe Mutomb", test$player_name)
test[d4, 53] <- "YES"
d5<-grep("Gary Payton", test$player_name)
test[d5, 53] <- "YES"
...
test[d8, 54] <- "YES"
d9<-grep("PJ Brown", test$player_name)
test[d9, 54] <- "YES"
d10<-grep("Clifford Robinson", test$player_name)
test[d10, 54] <- "YES"
#d11<-grep("Shaquille O'Neal", test$player_name)
#test[d11, 54] <- "YES"
#MVP 43
mvp<-grep("Allen Iverson", test$player_name)
test[mvp, 43] <- "YES"
#MIP 44
mip<-grep("Tracy McGrady", test$player_name)
test[mip, 44] <- "YES"
#6th man 45 
sixth<-grep("Aaron McKie", test$player_name)
test[sixth, 45] <- "YES"
#DPOY 46
dpoy<-grep("Dikembe Mutombo", test$player_name)
test[dpoy, 46] <- "YES"
#ROY 47
roy<-grep("Mike Miller", test$player_name)
test[roy, 47] <- "YES"
test<-format(test, digits = 11)

write.csv(test, file = "2002_data.csv")